...
io.on('connection', function (socket) {//We need to open the browser with the url HTTPS in order to user DialogFlow
 function sending(){//Intent confirmation sending, the user say YES for sending the form we'll generate a .txt with an order number
  runPy = new Promise(function(success, nosuccess) {//Run Python's script
  ...
  app.post('/', express.json(), function (req, res) {
    if (req.body.queryResult.action === "horary") {
      ...
      console.log(req.body.queryResult.queryText);
      let phone = req.body.queryResult.parameters.phone-number;
      reply = phone;
      socket.emit('bot reply', reply);//Here we will give the respose to the browser   
      res.json({
        "fulfillmentText": reply
      ...
      this.sending();//we call the function to generate the txt with the order number
      socket.emit('bot reply', reply);//Here we will give the respose to the browser     
      res.json({
        "fulfillmentText": reply
      });
